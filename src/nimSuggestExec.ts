...
export async function setNimSuggester() {
  ...
  if (process.env['PATH']) {
    ...
    let pathes = pathparts
      .map((dir) => path.join(dir, correctBinname(name)))
    ...
    const p = vscode.window.showQuickPick(items, {
      placeHolder: `current ${nimsuggest.path}`,
      matchOnDetail: true,
    });
    p.then((item: { detail: string }) => {
      if (!item || nimsuggest.path === item.detail) {
        return;
      }
      let nim = vscode.workspace.getConfiguration('nim');
      nim.update('nimsuggest', item.detail).then((_: any) => {
